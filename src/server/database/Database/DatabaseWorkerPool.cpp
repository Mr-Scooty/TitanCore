      ...
      _async_threads(0), _synch_threads(0)
{
    ...
    // The runtime client library only needs to satisfy the minimum version;
    // requiring an exact match with the compile-time headers breaks whenever
    // the distribution updates libmysqlclient.
    WPFatal(mysql_get_client_version() >= MIN_MYSQL_CLIENT_VERSION, "TrinityCore does not support MySQL versions below 8.0");
}
...
uint32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, uint8 numConnections)
{
    for (uint8 i = 0; i < numConnections; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql.driver", "TrinityCore does not support MySQL versions below 8.0");
            return 1;
        }
        else
        {
